    ...
    if tile_prefix == "items" and cur_mode != "remove":
        ...
        for line_num in range(len(cur_map.field[y][x].onload)):
            if cur_map.field[y][x].onload[line_num][:4].strip().lower() == "walk":
                try:
                    cur_map.field[y][x].onload[line_num] = "walk(" + walk_vals[temp_name] + ")"
                #default to walkable.
                except KeyError:
                    cur_map.field[y][x].onload[line_num] = "walk(1)"
    refresh_map()
    ...
    for tile_type in range(count):  #for each tile type.
        ...
        for y in range(mapsize_y):  #Find an example of that tile.
            for x in range(mapsize_x):
                ...
                continue  #bit confusing here; this breaks out of both loops
...
def map_menu():
    ...
    if tmp == "Map Mode":
        ...
        if tmp == -1:
            refresh_map()
            return
        if tmp == "Replace Tile (default)":
            set_mode("replace")
        elif tmp == "Add Tile (transparency)":
            set_mode("add")
        elif tmp == "Reset Tile (clear to background)":
            set_mode("remove")
        elif tmp == "Adjust walkable status":
            set_mode("walkable")
        elif tmp == "Edit Onload":
            set_mode("scripting")
        elif tmp == "Edit Action":
            set_mode("scripting2")
        elif tmp == "Add Overtile":
            set_mode("addover")
        elif tmp == "Copy Tile":
            set_mode("tilecopy")
    elif tmp == "Map Size":
        ...
        if tmp == -1:
            ...
            return
        if tmp == "Current Tile":
            change_tile = cur_tile
        elif tmp == "Water":
            change_tile = "water/water.png"
        elif tmp == "Dirt":
            change_tile = "underground/dirt.png"
        elif tmp == "Void":
            change_tile = "underground/void.png"
        elif tmp == "Grass":
            change_tile = "grass.png"
        elif tmp == "Desert":
            change_tile = "desert/desert_grass.png"
        set_bkg(change_tile)
    elif tmp == "Border":
        ...
        if tmp == "Current Tile":
            ...
            change_tile = "underground/rock.png"
        elif tmp == "Grass":
            change_tile = "grass.png"
        elif tmp == "Hills":
            change_tile = "hills/hills_n4.png"
        elif tmp == "Rock":
            change_tile = "rock.png"
        elif tmp == "Void":
            change_tile = "underground/void.png"
        elif tmp == "Water":
            change_tile = "water/water.png"
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    ...
                    move_down(portalsize)
                elif event.key == pygame.K_HOME:
                    move_left(portalsize)
                elif event.key == pygame.K_END:
                    move_right(portalsize)
                elif event.key == pygame.K_ESCAPE:
                    if ask_yesno("Quit the editor?"): sys.exit()
                    ...
                    try:
                        tile_dir_list(int(event.unicode) - 1)
                    except ValueError:
                        if event.unicode == "!":
                            tile_dir_list(13)
                        elif event.unicode == "@":
