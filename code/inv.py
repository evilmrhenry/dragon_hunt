...
def create_inv_display(screen_str):
    g.create_norm_box((tmp_x_base, tmp_y_base),
                      ((g.tilesize * main.mapsizex) / 2 - tmp_x_base, (g.tilesize * main.mapsizey +
                                                                       inv_canvas_height) / 2 + g.buttons[
                           "leave.png"].get_height() - tmp_y_base))
...
def refresh_equip():
    ...
    #Weapon
    if player.equip[0] != -1:
        draw_item(item.item[player.equip[0]].picturename,
                  0, 1, tmpx, tmpy, "equip")
    else:
        draw_item("items/weapon_eq.png", 0, 1, tmpx, tmpy, "equip")
    #Armor
    if player.equip[1] != -1:
        draw_item(item.item[player.equip[1]].picturename,
                  1, 1, tmpx, tmpy, "equip")
    else:
        draw_item("items/armor_eq.png", 1, 1, tmpx, tmpy, "equip")
    #Shield
    if player.equip[2] != -1:
        draw_item(item.item[player.equip[2]].picturename,
                  2, 1, tmpx, tmpy, "equip")
    else:
        draw_item("items/shield_eq.png", 2, 1, tmpx, tmpy, "equip")
    #Helmet
    if player.equip[3] != -1:
        draw_item(item.item[player.equip[3]].picturename,
                  1, 0, tmpx, tmpy, "equip")
    else:
        draw_item("items/helmet_eq.png", 1, 0, tmpx, tmpy, "equip")
    #Gloves
    if player.equip[4] != -1:
        draw_item(item.item[player.equip[4]].picturename,
                  2, 0, tmpx, tmpy, "equip")
    else:
        draw_item("items/gloves_eq.png", 2, 0, tmpx, tmpy, "equip")
    #Boots
    if player.equip[5] != -1:
        draw_item(item.item[player.equip[5]].picturename,
                  1, 2, tmpx, tmpy, "equip")
    else:
...
def refresh_skill(screen_str):
    ...
    if len(player.skill) <= curr_item:
        helptext = ""
                    ...
                    player.skill[curr_item][1] <= 4:
        helptext = ""
    else:
        helptext = (player.skill[curr_item][0] + " (" +
